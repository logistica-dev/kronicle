...
class ColumnFilter:
    ...
    def to_sql(self, idx: int) -> tuple[str, list[Any]]:
        sql_col = self.col.sql_identifier
        condition = self._condition(idx)

        if self.col.is_json:
            path_sql, last_key = self.col.json_path_sql
            sql = f"({sql_col}{'->' + path_sql if path_sql else ''}->>'{last_key}') {condition}"
            return sql, self._sql_params
        return f"{sql_col} {condition}", self._sql_params
...
class RangeFilter(ColumnFilter):
    def __init__(self, resolved_col: ResolvedColumn, value: Any):
        if resolved_col.col_type.name == "dict":
            raise ValueError(
                f"Text filters (col/any) not supported on JSON field "
                f"'{resolved_col.col_name}.{'.'.join(resolved_col.subkeys) if resolved_col.subkeys else ''}'"
            )
        if resolved_col.col_type.name == "list":
            raise ValueError(f"Text filters (col/any) not supported on list field '{resolved_col.col_name}'")
        super().__init__(resolved_col, resolved_col.col_type.normalize_value(value))

    @property
    def sql_cast(self) -> str:
        if self.col.col_type.name in {"int", "float"}:
            return "::numeric"
        if self.col.col_type.name in {"datetime", "timestamptz", "timetz"}:
            return "::timestamptz"
        return ""

    def to_sql(self, idx: int) -> tuple[str, list[Any]]:
        return f"{self.col.sql_identifier}{self.sql_cast} {self._condition(idx)}", self._sql_params
...
class AnyFilter(ColumnFilter):
    ...
    def __init__(self, resolved_col: ResolvedColumn, value: Any):
        ...
        if resolved_col.col_type.name == "dict":
            ...
            values = [stripped for v in value.split(",") if (stripped := v.strip())]

        elif isinstance(value, str):
            log_d(here, value, "is str")

            log_d(here, resolved_col.col_name, f"is {resolved_col.col_type.name}")
            values = [
                resolved_col.col_type.normalize_value(stripped_val)
                for v in value.split(",")
                if (stripped_val := v.strip())
            ]
            log_d(here, "splitted values:", values)

        elif isinstance(value, (list, tuple)):
            if not resolved_col.is_collection:
                values = [resolved_col.col_type.normalize_value(v) for v in value]
            else:
