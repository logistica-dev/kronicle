...
class ZoneAccessProfileRepository(KronicleRepository[ZoneAccessProfile]):
    ...
    @log_repo_error
    def create(self, db: Session, *, role_id: UUID, zone_id: UUID, name: str | None = None) -> ZoneAccessProfile:
        profile = ZoneAccessProfile(role_id=role_id, zone_id=zone_id, name=name)
        db.add(profile)
        db.flush()
        return profile
