# tests/unit/repo/rbac/links/test_row_access_profile_repo.py
...
@pytest.fixture
def repo():
    return RowAccessProfileRepository()
...
class TestGetByRoleAndRow:
    def test_returns_profile(self, repo, mock_db):
        expected = MagicMock()
        mock_db.execute.return_value.scalar_one_or_none.return_value = expected
        result = repo.get_by_role_and_row(mock_db, role_id=uuid4(), row_id=uuid4())
        assert result is expected

    def test_returns_none_when_not_found(self, repo, mock_db):
        mock_db.execute.return_value.scalar_one_or_none.return_value = None
        assert repo.get_by_role_and_row(mock_db, role_id=uuid4(), row_id=uuid4()) is None


class TestCreate:
    def test_adds_and_flushes_profile(self, repo, mock_db):
        result = repo.create(mock_db, role_id=uuid4(), row_id=uuid4(), name="test_row_profile")
        assert isinstance(result, RowAccessProfile)
        mock_db.add.assert_called_once()
        mock_db.flush.assert_called_once()

    def test_creates_profile_with_no_name(self, repo, mock_db):
        result = repo.create(mock_db, role_id=uuid4(), row_id=uuid4())
